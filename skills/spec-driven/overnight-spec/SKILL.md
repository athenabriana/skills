---
name: overnight-spec
description: Run ONE pre-written spec task overnight as a desired-state cron-once loop, ending at a draft PR you review in the morning — never a continuous while-true, never an auto-merge. Reloads a committed .specs/ task list, picks the single next task deterministically, implements only it via tlc-spec-driven's Execute, runs the local gate, commits to a claude/ branch, opens a DRAFT PR, and stops. Use when the user says "run the overnight spec queue", "do one small task overnight", or "set up the nightly draft-PR loop". Do NOT use without a completed .specs/ spec (run /tlc-spec-driven first), for open-ended "improve the codebase" work, or to merge (that stays human).
license: MIT
metadata:
...
# Overnight Spec (cron-once, draft-PR only)
...
a Cloud Routine. It builds on `/tlc-spec-driven` (the spec + Execute) and
`/gh-guardrails` (the safety layer) — install both.

## Prerequisites

- A completed `tlc-spec-driven` feature with a committed `.specs/features/<feature>/tasks.md`. A Cloud Routine clones fresh with no local files, so `.specs/` MUST be committed for the run to see it. If there is no ready task list, the run NO-OPs and reports.
...
2. **Pick ONE task.** `python scripts/next_task.py --tasks .specs/features/<feature>/tasks.md` → the first unchecked task `{id, title, files}` (zero tokens; the script decides which, never the model). If `next` is null, no-op and report "backlog empty".
3. **Implement ONLY that task** via tlc-spec-driven's Execute phase, honoring its safety valve: if implementing the one task reveals **>5 emergent steps**, STOP, formalize them into `tasks.md`, and report — do not expand scope.
...
- **DO NOT** assume a local `.specs/` is visible to a cloud routine — it must be committed.
...
The Cloud Routine setup: commit `.specs/`, withhold merge scope, the guard hook + probe, the draft-PR-only prompt with the scope ceiling, and the cost cap.
