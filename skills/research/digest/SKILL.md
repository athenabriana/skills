...
2. **Research each entry** via the existing `/research-topic` engine (decompose → parallel sub-agents → cited synthesis, dedupe sources, flag disagreements). Keep the fan-out **small** (2–3, not 4) for a scheduled run until cost is measured.
