...
### Companion skill: /gh-guardrails (unattended path only)
The no-merge PreToolUse hook, worktree isolation, and the guard self-test are shared from `/gh-guardrails` rather than duplicated here. Install it alongside for any unattended routine: its `scripts/guard_irreversible.py` is the canonical hook (Windows-aware; denies merge / approve / force-push / branch-delete / destructive wipes) and its `references/settings.example.json` is the `.claude/settings.json` block to commit into the target repo. The supervised path (read-only digest + human merge) does not need it.
