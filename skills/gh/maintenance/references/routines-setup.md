# Unattended setup — running gh-maintenance as Cloud Routines
...
- [ ] `/gh-guardrails`'s `references/settings.example.json` is committed to the
      **target repo's** `.claude/settings.json` so its `guard_irreversible.py` runs
      as a PreToolUse tripwire (install `/gh-guardrails` alongside this skill).
- [ ] **Probe it in cloud:** do one `Run now` where the prompt asks the agent to
      ...
      denies it. It is **unverified** whether routines honor repo-committed hooks —
...
- **Prompt (self-contained):** run `gh-maintenance` Phase 1→3 for the triggering
...
- **Prompt:** run `gh-maintenance` Phase 1→4 across **all** open PRs + alerts +
