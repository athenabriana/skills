...
running the trio overnight: a routine fires, sets `ATH_UNATTENDED`, and runs
`/ath:implement` → `/ath:ship` against one shaped brief — building the whole
...
desk, with the `ATH_UNATTENDED` frame changing their behavior (no questions,
fixed draft-PR destination, capped retries). The frame is injected by the
SessionStart hook when `ATH_UNATTENDED` is truthy.
...
- **The work must run while your laptop is closed.** A session `/loop` expires
  ...
  `/ath:implement` directly — you don't need a routine.
- **You have a committed, validated brief.** The fresh clone can only see what's
  in git, so `.ath/tasks/<slug>/shape.md` (brief + `## tasks` checklist) must be
...
> Set `ATH_UNATTENDED=1`. Run `/ath:implement` against
> `.ath/tasks/<slug>/shape.md` in this repo: build every unchecked task in the
...
> chain into `/ath:ship` — open a **DRAFT** PR against `<base>` and watch it to
