...
- **Shape before building anything non-trivial.** For a real feature, reach for `/ath:shape-idea` first — develop a draft, loop through the gray areas as questions, converge on a brief, _then_ build. Skip it for tiny mechanical changes; just do those.
