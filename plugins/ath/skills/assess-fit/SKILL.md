...
Read `.ath/tasks/<slug>/shape.md` first. If `## problem` / `## hypothesis` exist
(from `/ath:frame-problem`), echo the trio and build on it. If they don't,
suggest `/ath:frame-problem` once — non-blocking; if the user wants to continue
...
Write into the same `.ath/tasks/<slug>/shape.md`, accreting alongside the trio:
...
- A legal-sensitive bet → `/ath:legal-lens` over the brief before committing.
