...
### Companion skill: /ath:loop-guardrails (unattended path only)
The no-merge PreToolUse hook, worktree isolation, and the guard self-test are shared from `/ath:loop-guardrails` rather than duplicated here. Install it alongside for any unattended routine: its `scripts/guard_irreversible.py` is the canonical hook (Windows-aware; denies merge / approve / force-push / branch-delete / destructive wipes) and its guard hook auto-activates with the `ath` plugin (no manual settings.json). The supervised path (read-only digest + human merge) does not need it.
