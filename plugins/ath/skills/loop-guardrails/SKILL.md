...
2. **Deterministic hook (harness-enforced).** `scripts/guard_irreversible.py` denies tier-2 ops (merge, approve, force-push, protected-branch push, branch delete, untrusted deploy, recursive wipe) regardless of model cooperation. It ships as the **`ath` plugin's `PreToolUse` hook** (`plugins/ath/hooks/hooks.json`, wired via `${CLAUDE_PLUGIN_ROOT}`) and **auto-activates when the plugin is enabled** — no manual `settings.json` edit.
3. **Active self-test (fail-closed).** `scripts/selftest_guard.py` fires known-bad probes through the hook and asserts each is denied. Run it as the FIRST step of any unattended run; if it fails, ABORT — an unwired hook is the ultimate bypass. For a Cloud Routine, do a one-off probe `Run now` to confirm the plugin hook actually fires there (**unverified** — if the probe is not denied, rely solely on layer 1).
...
The `ath` plugin's `PreToolUse` wiring that runs `guard_irreversible.py` via `${CLAUDE_PLUGIN_ROOT}`. Auto-activates when the plugin is enabled — nothing to commit by hand.
