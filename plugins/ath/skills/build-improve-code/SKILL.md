---
name: build-improve-code
description: Improve the quality of the code this branch changed — reuse, simplification, dead weight, efficiency, altitude, consistency — and apply the cleanups, quality only and strictly behavior-preserving, with a hard regression guard (verify behavior, re-run checks after every edit). Use when the user says "improve this code", "clean this up", "simplify the diff", "tidy this code", "reduce complexity", or "refactor for clarity". Do NOT use to find or fix bugs (that is correctness review — use /ath:build-ship-pr), to open a PR (use /ath:build-ship-pr), or to touch code this branch did not change.
license: Apache-2.0
metadata:
...
consistent **without changing behavior**. The standalone version of `ath:build-ship-pr`'s
...
review, that is Pass 1 of `/ath:build-ship-pr` — use that skill. Mixing the two makes
both passes worse.
...
with `ath:build-ship-pr` Pass 2 so the standalone and PR-finalization flows judge a diff
the same way.
