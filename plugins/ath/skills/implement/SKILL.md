---
name: implement
description: Implement a validated `.ath/tasks/<slug>/shape.md` brief in the working tree — build the slices, run the local gate, commit, and stop ready to ship. Supervised and at-your-desk (the daytime sibling of the overnight loop-night-shift). Use when the user says "implement the brief", "build the shaped tasks", "build it", "implement this", or right after /ath:shape. Do NOT use to shape an idea first (use /ath:shape), to open or green a PR (use /ath:ship), or for an unattended overnight run (use /ath:night-shift).
license: MIT
metadata:
...
Build the validated, shaped work in the working tree — supervised, all at once — taking `.ath/tasks/<slug>/shape.md` from a brief to code that's ready to ship. The at-desk executor; for unattended one-task-per-night work ending at a draft PR, use `/ath:night-shift`.
