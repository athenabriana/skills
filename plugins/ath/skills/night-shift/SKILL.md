...
- A `/ath:shape-idea` brief committed as `.shape/<slug>.md` with a `## tasks` checklist. A Cloud Routine clones fresh with no local files, so `.shape/` MUST be committed for the run to see it. If there is no ready task list, the run NO-OPs and reports.
