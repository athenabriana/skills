...
a Cloud Routine. It builds on `/ath:shape` (the brief + sliced tasks) and
`/ath:guardrails` (the safety layer) — install both.

## Prerequisites

- A `/ath:shape` brief committed as `.shape/<slug>.md` with a `## tasks` checklist. A Cloud Routine clones fresh with no local files, so `.shape/` MUST be committed for the run to see it. If there is no ready task list, the run NO-OPs and reports.
- `/ath:guardrails` installed and its self-test passing (the first step below).

## Workflow (one iteration = one task)

1. **Guard self-test (gate).** Run `ath:guardrails`' `selftest_guard.py`. If it fails, ABORT — do not run unguarded.
...
3. **Implement ONLY that task**, honoring the re-shape safety valve: if implementing the one task reveals **>5 emergent steps**, STOP, formalize them into the `## tasks` section (or hand back to `/ath:shape`), and report — do not expand scope.
...
5. **Run the local gate** (lint/format/typecheck/tests) until clean, capped like ath:fix-pr Phase 5: at most **3 log-gated retries**, and only on a known-flake signature — a real failure short-circuits to "halt + report".
6. **Commit to a `claude/` (or `auto/`) branch, open a DRAFT PR** (chain to `/ath:open-pr` for title/body), check the task's box in the `## tasks` section of `.shape/<slug>.md`, and **STOP**. You review and merge in the morning.
...
- **Draft-PR only, never merge.** The land step stays human. Withhold merge capability from the routine (`ath:guardrails/references/autonomy-boundary.md`); the PreToolUse hook is the backstop.
