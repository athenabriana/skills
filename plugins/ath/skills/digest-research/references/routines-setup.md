# Unattended setup — ath:digest-research as a daily Cloud Routine
...
- **Committed digest branch:** the routine writes `digests/YYYY-MM-DD.md` on a
  `claude/ath:digest-research` branch and opens/updates a draft PR. Durable,
...
- Trigger: schedule, daily, an off-the-hour weekday-morning slot (1-hour minimum
  is easily met).
- Fan-out: keep `/ath:research-topic` at **2–3 sub-agents** for the scheduled
