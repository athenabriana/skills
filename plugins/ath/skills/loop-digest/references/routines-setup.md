# Unattended setup — ath:loop-digest as a daily Cloud Routine
...
- **Committed digest branch:** the routine writes `digests/YYYY-MM-DD.md` on a
  `claude/ath:loop-digest` branch and opens/updates a draft PR. Durable,
...
- Optionally commit the `/ath:loop-guardrails` PreToolUse hook so any stray
  write/merge command is denied as defense-in-depth.

## 4. Schedule & cost

- Trigger: schedule, daily, an off-the-hour weekday-morning slot (1-hour minimum
  is easily met).
- Fan-out: keep `/ath:util-topic` at **2–3 sub-agents** for the scheduled
