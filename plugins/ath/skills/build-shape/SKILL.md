...
- **Daytime:** "Brief saved at `.shape/<slug>.md`. To build it: `/ath:build-implement`, then `/ath:build-ship-pr` — both load this brief as the intent."
- **Overnight:** "Commit `.shape/<slug>.md`, then schedule `/ath:loop-night-shift` — it builds one slice per run."
