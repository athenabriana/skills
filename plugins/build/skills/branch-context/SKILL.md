---
name: branch-context
description: Gather and summarize all changes on the current branch compared to main. Use when user says "compare against main", "branch context", "what changed on this branch", "summarize my branch", "diff against main", "review my changes", or "what did I do on this branch". Do NOT use to fix or green a PR (use /build:fix-pr), open a PR (use /build:open-pr), or improve the diff's quality (use /build:improve-code) — this only summarizes, it never edits.
license: MIT
metadata:
