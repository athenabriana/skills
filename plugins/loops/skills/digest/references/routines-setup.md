# Unattended setup — loops:digest as a daily Cloud Routine
...
- **Committed digest branch:** the routine writes `digests/YYYY-MM-DD.md` on a
  `claude/loops:digest` branch and opens/updates a draft PR. Durable,
...
- Optionally commit the `/loops:guardrails` PreToolUse hook so any stray
...
- Trigger: schedule, daily, an off-the-hour weekday-morning slot (1-hour minimum
  is easily met).
- Fan-out: keep `/utils:topic` at **2–3 sub-agents** for the scheduled
