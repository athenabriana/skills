...
### Companion skill: /loops:guardrails (unattended path only)
The no-merge PreToolUse hook, worktree isolation, and the guard self-test are shared from `/loops:guardrails` rather than duplicated here. Install it alongside for any unattended routine: its `scripts/guard_irreversible.py` is the canonical hook (Windows-aware; denies merge / approve / force-push / branch-delete / destructive wipes) and its guard hook auto-activates with the `loops` plugin (no manual settings.json). The supervised path (read-only digest + human merge) does not need it.
