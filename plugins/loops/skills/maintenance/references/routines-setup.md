# Unattended setup — running loops:maintenance as Cloud Routines
...
- [ ] The **`loops` plugin** is installed — its `PreToolUse` guard hook
...
- **Prompt (self-contained):** run `loops:maintenance` Phase 1→3 for the triggering
...
- **Prompt:** run `loops:maintenance` Phase 1→4 across **all** open PRs + alerts +
