...
2. **Deterministic hook (harness-enforced).** `scripts/guard_irreversible.py` denies tier-2 ops (merge, approve, force-push, protected-branch push, branch delete, untrusted deploy, recursive wipe) regardless of model cooperation. It ships as the **`loops` plugin's `PreToolUse` hook** (`plugins/loops/hooks/hooks.json`, wired via `${CLAUDE_PLUGIN_ROOT}`) and **auto-activates when the `loops` plugin is enabled** — no manual `settings.json` edit.
...
The `loops` plugin's `PreToolUse` wiring that runs `guard_irreversible.py` via `${CLAUDE_PLUGIN_ROOT}`. Auto-activates when `loops` is enabled — nothing to commit by hand.
